...
export async function dailyFunnel(days = 30): Promise<DailyRow[]> {
  ...
  const { data } = await admin()
    .from('v_daily_funnel').select('*').gte('day', from).order('day');
...
export async function stageCounts(days = 30) {
  ...
  const { data } = await admin()
    .from('contacts').select('stage').gte('first_seen_at', from).limit(10000);
...
export async function overview(days = 30) {
  ...
  const [contacts, engaged, orders, spendRes, needsHuman, botHandled, pendingTasks, aiRuns] =
    await Promise.all([
      db.from('contacts').select('id', { count: 'exact', head: true }).gte('first_seen_at', fromIso),
      db.from('conversations').select('id', { count: 'exact', head: true }).gt('inbound_count', 1),
      db.from('orders').select('amount').gte('created_at', fromIso).in('status', ['confirmed', 'delivered']),
      db.from('ad_insights').select('spend').gte('date', fromDay),
      db.from('conversations').select('id', { count: 'exact', head: true }).eq('status', 'needs_human'),
      db.from('conversations').select('id', { count: 'exact', head: true }).eq('last_reply_by', 'bot'),
      db.from('follow_ups').select('id', { count: 'exact', head: true }).eq('status', 'pending'),
      db.from('ai_runs').select('action').gte('created_at', fromIso).limit(10000),
    ]);

  const revenue = (orders.data ?? []).reduce((s, o) => s + Number(o.amount), 0);
  ...
  const { count: noConvo } = await db
    .from('conversations').select('id', { count: 'exact', head: true }).lte('inbound_count', 1);
...
export async function adPerformance() {
  const { data } = await admin()
    .from('v_ad_performance').select('*').order('spend', { ascending: false }).limit(100);
  return data ?? [];
}
...
export async function conversationList(filter: string) {
  let q = admin()
    .from('conversations')
    .select('*, contacts(id,name,psid,stage,phone,profile_pic,source_ad_id)')
...
export async function conversationDetail(id: string) {
  ...
  const { data: messages } = await db
    .from('messages').select('*').eq('conversation_id', id).order('sent_at').limit(200);
  const { data: events } = await db
    .from('lead_events').select('*').eq('contact_id', convo.contact_id)
...
export async function followUpQueue() {
  const { data } = await admin()
    .from('follow_ups')
    .select('*, contacts(id,name,psid,stage,phone,last_inbound_at)')
    .eq('status', 'pending')
    .order('priority')
    .order('due_at')
    .limit(200);
