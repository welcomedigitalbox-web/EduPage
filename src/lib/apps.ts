...
//
// Sign-in is shared across these subdomains: a session cookie on
// .edubabyhouse.store is readable by all of them, so one login covers the lot.
// The POS owns the sign-in screen.
...
export function safeNext(raw: string | null): string | null {
  if (!raw) return null;
  try {
    const url = new URL(raw, APP_URL.onlineorder);
    const ok =
      url.protocol === 'https:' &&
